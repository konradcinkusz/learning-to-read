...
\item \textbf{An activity}, which always ends with a picture to colour:
  a word the child has just read, or on Fridays a scene of the week. The
...
{\small
\begin{itemize}[leftmargin=6mm, itemsep=1pt, topsep=2pt]
\item \textbf{\lblColorea}: the picture of a word of the day, big: the
  simplest way to check that it has been understood, not only sounded out.
\item \textbf{\lblCompleta}: colour the picture and draw what is missing
  (a tail, the candles, the sun).
...
\item \textbf{\lblAdivina}: the grown-up reads a riddle; the child
  colours the picture that answers it (from winter, and writes it).
\item \textbf{\lblUne}: join each word to its picture (in summer, other
  pairs: opposites, rhymes\ldots).
